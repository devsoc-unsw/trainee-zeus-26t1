...
**Architecture:** Two new modules under `backend/app/game/`: `scoring.py` (Gemini free tier — calls `score_chain`) and `code_execution.py` (Judge0 — calls `run_code`). New Pydantic models (`ChainScore`, `TestCase`, `TestResult`) live in the existing `schemas.py`. `_finish_game` gains a best-effort `_score_chains_safe` helper that catches `NotImplementedError` so the reveal still works without scoring.

**Tech Stack:** Python 3.12, FastAPI, Pydantic v2, asyncio. No new dependencies in this pass — the teammate filling in `score_chain` will add the `google-genai` SDK, and the teammate filling in `run_code` will add an HTTP client.
